...
- **(0.5 h)** Notes: `notes/fhir-fundamentals.md`. Export 2–3 resources as JSON files into `day1-fhir/fixtures/` — Day 7 loads these into Postgres.

## Day 2 — Schema design: keys, normalization, constraints + capstone v1 (3 h)

- **(0.5 h)** Run `day2-design/exercise-keys.sql`; answer its questions. Generic: primary/candidate/natural/surrogate keys, tradeoffs (identity vs uuid).
...
- **(1 h)** **Capstone:** design WellSync-lite v1 — Mermaid ER diagram + DDL in `day2-design/capstone/`: patients, practitioners, encounters, observations. Model on the FHIR resources you saw Day 1. Short `DESIGN.md`: justify each key + constraint.
...
- **(1 h)** Two-terminal labs (commit scripts to `day3-transactions/`): (a) demonstrate non-repeatable read at RC, gone at RR; (b) write skew at RR, fixed by Serializable — observe serialization failure, write retry loop; (c) force a deadlock, read the error.
...
- **(1 h)** Lab: 5 realistic capstone queries ("latest vitals for patient", "encounters this week per practitioner", …). Make each fast; commit before/after plans as proof to `day4-indexing/`.
...
- **(1 h)** **RLS** (healthcare-critical): policy so a practitioner sees only own patients' rows. Test as different roles with `SET ROLE`. Commit to `day5-auth-rls/`.
...
- **(1.5 h)** Disaster drill: `pg_dump` capstone → drop database → restore → verify counts. Then PITR concept + lab-lite: deliberate bad `DELETE`, restore dump from before. Logical vs physical backup tradeoffs. Script into `day6-backup-maintenance/`. Principle: untested backup = no backup.
...
- **(1 h)** Migrations: forward-only vs up/down; **expand-and-contract** for zero downtime; never edit applied migration. Postgres gotchas: DDL locks, `CREATE INDEX CONCURRENTLY`, safe `NOT NULL` addition. Retrofit capstone as ordered migration series (plain SQL + `dbmate` or bare psql runner) in `day7-migrations-fhir/migrations/`.
...
day1-fhir/             # FHIR fixtures exported from fhir-candle
day2-design/           # keys exercise, normalization, capstone DDL
day3-transactions/     # isolation/locking lab scripts
day4-indexing/         # tuning evidence (before/after plans)
day5-auth-rls/         # roles, GRANT, RLS policies
day6-backup-maintenance/  # backup scripts, OPERATIONS.md runbook
day7-migrations-fhir/  # migration series + hybrid FHIR store
